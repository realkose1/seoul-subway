...
function toTrip(cacheRow) {
  ...
  if (cacheRow.date === LOCK_KEY || cacheRow.date.startsWith(HIST_PREFIX)) return null;
...
async function listTrips() {
  ...
  const rows = await sbSelect(CACHE_TABLE, `date=like.${encodeURIComponent(PREFIX + "*")}&date=not.like.${encodeURIComponent(HIST_PREFIX + "*")}&select=date,events,updated_at`);
...
module.exports = {
  PREFIX, LOCK_KEY, HIST_PREFIX, CACHE_TABLE, ROW_KEYS, getHist, saveHist,
