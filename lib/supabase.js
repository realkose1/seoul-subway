...
function toTrip(cacheRow) {
  ...
  if (cacheRow.date === LOCK_KEY || cacheRow.date === PARKED_KEY || cacheRow.date === SCAN_KEY || cacheRow.date.startsWith(HIST_PREFIX)) return null;
...
async function listTrips() {
  ...
  const rows = await sbSelect(CACHE_TABLE, `date=like.${encodeURIComponent(PREFIX + "*")}&date=not.like.${encodeURIComponent(HIST_PREFIX + "*")}&date=neq.${encodeURIComponent(PARKED_KEY)}&date=neq.${encodeURIComponent(SCAN_KEY)}&select=date,events,updated_at`);
...
module.exports = {
  PREFIX, LOCK_KEY, HIST_PREFIX, PARKED_KEY, SCAN_KEY, CACHE_TABLE, ROW_KEYS, getHist, saveHist, getParked, saveParked,
  getScan, saveScan,
