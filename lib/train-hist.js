...
const { normStation, recptMs, dedupByTrainNo, onLine } = require("./trip-state");
...
/* 피드 목록 한 번을 이력에 쌓는다. 수신시각이 없으면 now 를 쓴다.
   이력은 노선별(hist 하나 = 노선 하나, 열차번호로 묶음) — line 을 주면 다른 노선명이 찍힌 행은 섞지 않는다(onLine) */
function histObserve(hist, list, now = Date.now(), line = null) {
  if (!hist || !hist.trains) return hist;
  for (const t of dedupByTrainNo(list)) {
    if (line && !onLine(t, line)) continue;
...
/* ── 운행 대기 등록부(parked registry) ──
   실측(2026-09-24): 5197 이 11:42 부터 상일동에 서 있었는데(14:41 에도 그대로), 13:00~14:39 사이 아무도 5호선을
   관측하지 않아 이력(60분 보관)이 비었다 → 14:39 에 처음 본 5197 이 '방금 도착'으로 보여 '승강장 도착'으로 두 번 제안됐다.
   → 운행 대기로 판정된 열차를 이력과 따로 6시간 남긴다. 같은 노선·번호 열차가 같은 역에 출발(2) 아닌 상태로
      (다시) 보이면 곧바로 운행 대기로 보고, 그 방문의 시작 시각을 등록부의 since 로 되돌린다(→ '… · 180분째').
      다른 역에서 보이거나 그 역에서 출발(2)하면 즉시 지운다.
   모양: reg = { trains: { "노선|번호": {line, no, stn, since, lastSeen} }, lines: { 노선: 마지막 활동 ms }, at }
   서버 sf_cache 'ssl:la:parked' 한 행, 웹 localStorage 'trainParked' 가 같은 모양·같은 규칙(index.html parkReg*). */
const PARKED_REG_KEEP_MS = 6 * 3600 * 1000;
const emptyReg = () => ({ trains: {}, lines: {}, at: 0 });
const regKey = (line, no) => String(line || "") + "|" + String(no == null ? "" : no);

/* 노선 이력 h 를 보고 등록부를 갱신하고, 등록부의 정차 시작 시각을 이력에 되돌려 쓴다. 등록부가 바뀌었으면 true.
   (1) 등록된 열차가 등록 뒤 다른 역에서 보였거나 그 역에서 출발(2)했으면 지운다
   (2) 같은 역에 출발 아닌 상태로 있으면 lastSeen 을 늘리고, 그 방문을 [since, …] 로 넓힌다(새 기록 수 ≥ 2) → parkedInfo 가 곧바로 잡는다
   (3) parkedInfo 로 운행 대기인 열차는 등록(같은 역이면 since 는 더 이른 쪽) */
function regUpdate(reg, line, h, now = Date.now()) {
  if (!reg || !reg.trains || !line || !h || !h.trains) return false;
  let changed = false;
  for (const no of Object.keys(h.trains)) {
    const e = h.trains[no];
    if (!e || !Array.isArray(e.v) || !e.v.length) continue;
    const k = regKey(line, no), lv = e.v[e.v.length - 1];
    let ent = reg.trains[k];
    if (ent && now - (Number(ent.lastSeen) || 0) > PARKED_REG_KEEP_MS) { delete reg.trains[k]; ent = null; changed = true; }
    if (ent) {
      const since = Number(ent.since) || 0, lastSeen = Number(ent.lastSeen) || 0;
      const moved = e.v.some((x) => x[0] !== ent.stn && Number(x[2]) > since);
      const departed = lv[0] === ent.stn && lv[3] === "2" && Number(lv[2]) >= lastSeen;
      if (moved || departed) { delete reg.trains[k]; ent = null; changed = true; }
      else if (lv[0] === ent.stn && lv[3] !== "2") {
        if (Number(lv[2]) > lastSeen) { ent.lastSeen = Number(lv[2]); changed = true; }
        if (since && since < Number(lv[1])) lv[1] = since;
        lv[4] = Math.max(Number(lv[4]) || 1, 2);
      }
    }
    const pi = parkedInfo(e);
    if (pi) {
      const cur = reg.trains[k];
      const nx = cur && cur.stn === pi.stn
        ? { line, no: String(no), stn: pi.stn, since: Math.min(Number(cur.since) || pi.since, pi.since), lastSeen: Math.max(Number(cur.lastSeen) || 0, pi.last) }
        : { line, no: String(no), stn: pi.stn, since: pi.since, lastSeen: pi.last };
      if (!cur || cur.stn !== nx.stn || cur.since !== nx.since || cur.lastSeen !== nx.lastSeen) { reg.trains[k] = nx; changed = true; }
    }
  }
  if (changed) reg.at = Math.max(Number(reg.at) || 0, now);
  return changed;
}
/* 6시간 넘게 못 본 열차·활동 기록을 지운다. 지운 게 있으면 true */
function regPrune(reg, now = Date.now()) {
  if (!reg || !reg.trains) return false;
  let changed = false;
  for (const [k, ent] of Object.entries(reg.trains)) {
    if (!ent || !ent.line || ent.no == null || !ent.stn || now - (Number(ent.lastSeen) || 0) > PARKED_REG_KEEP_MS) { delete reg.trains[k]; changed = true; }
  }
  if (reg.lines) for (const [ln, t] of Object.entries(reg.lines)) if (now - (Number(t) || 0) > PARKED_REG_KEEP_MS) { delete reg.lines[ln]; changed = true; }
  return changed;
}
/* src 를 dst 에 합친다 — 같은 열차·같은 역이면 since 는 이른 쪽·lastSeen 은 늦은 쪽, 역이 다르면 더 최근에 본 쪽 */
function regMerge(dst, src) {
  if (!dst || !dst.trains || !src) return dst;
  const list = Array.isArray(src) ? src : Object.values((src && src.trains) || {});
  for (const s of list) {
    if (!s || !s.line || s.no == null || !s.stn) continue;
    const k = regKey(s.line, s.no), d = dst.trains[k];
    const e = { line: String(s.line), no: String(s.no), stn: String(s.stn), since: Number(s.since) || 0, lastSeen: Number(s.lastSeen) || 0 };
    if (!d) dst.trains[k] = e;
    else if (d.stn === e.stn) { d.since = Math.min(Number(d.since) || e.since, e.since || Number(d.since)); d.lastSeen = Math.max(Number(d.lastSeen) || 0, e.lastSeen); }
    else if (e.lastSeen > (Number(d.lastSeen) || 0)) dst.trains[k] = e;
  }
  if (!Array.isArray(src) && src.lines) {
    dst.lines = dst.lines || {};
    for (const [ln, t] of Object.entries(src.lines)) dst.lines[ln] = Math.max(Number(dst.lines[ln]) || 0, Number(t) || 0);
  }
  dst.at = Math.max(Number(dst.at) || 0, Number(src.at) || 0);
  return dst;
}
/* 한 노선의 등록 열차 목록(op=hist 응답용) */
const regForLine = (reg, line) => Object.values((reg && reg.trains) || {}).filter((e) => e && e.line === line);
function regUnpack(o) {
  const r = emptyReg();
  if (!o || typeof o !== "object") return r;
  regMerge(r, { trains: o.trains || {}, lines: o.lines || {}, at: o.at });
  return r;
}
...
module.exports = {
  ...
  PARKED_REG_KEEP_MS, emptyReg, regKey, regUpdate, regPrune, regMerge, regForLine, regUnpack,
};
