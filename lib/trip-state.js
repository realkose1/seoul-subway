...
/* 종착역 표기 정규화 — 피드의 "응암순환(상선)", "신내행" 등을 역명으로(index.html resolve() 와 같은 규칙) */
function normTerm(s) {
  let n = normStation(s);
  n = n.replace(/(종착|지선|순환|행)$/, "").trim();
  if (n.length > 1 && n.endsWith("역")) n = n.slice(0, -1);
  return n;
}
 ...
 * 조건: 환승역에 있거나(statnNm == 환승역, 진입 "0" 포함) 환승역 직전 역(최대 2정거장, 진입·도착·전역출발 0/1/3)에서
 *       다가오는 중이고, 방향이 다음 구간과 맞을 것.
 * 방향 판정(웹 tripTrackPayload 가 구간마다 보내는 근거):
 *   - loopDir("0"/"1"): 순환선 본선 구간 — 열차의 updnLine 이 같아야 한다(offTerms = 지선 역이 종착이면 제외)
 *   - dirTerms: 승차역에서 그 역으로 가는 길이 이번 구간 종료역을 지나는 역들(= 이 방향 종착 후보, 구간 역 목록 밖 포함)
 *   - 그리고 구간 역 목록 안에서 환승역보다 뒤에 있는 역(예전 방식 — dirTerms 가 없는 옛 웹도 그대로 동작)
 *   before: 승차역 직전 역 사슬 [[직전, 그 전], …](분기마다 하나) — 다가오는 열차 판정용
 * 판단할 수 없으면 추측하지 않는다(computeRow 가 시간 기준으로 넘어간다).
 *
 * opts.skip(trainNo) 가 true 인 열차(운행 대기로 판정된 열차 등)는 고르지 않는다.
 * 여러 대면 환승역에 가까운 열차(환승역 도착 > 진입 > … > 2정거장 전)를 고른다.
 *
...
function pickNextTrain(track, list, now = Date.now(), opts = {}) {
  ...
  const after = new Set(nNorm.slice(xi + 1));
  const dirSet = new Set((Array.isArray(nextLeg.dirTerms) ? nextLeg.dirTerms : []).map(normStation).filter(Boolean));
  const offSet = new Set((Array.isArray(nextLeg.offTerms) ? nextLeg.offTerms : []).map(normStation).filter(Boolean));
  const loopDir = nextLeg.loopDir === "0" || nextLeg.loopDir === "1" ? nextLeg.loopDir : null;
  /* 직전 역 → {hop, approach(먼 역 → 가까운 역, 승차역 제외)} */
  const before = new Map();
  for (const chain of Array.isArray(nextLeg.before) ? nextLeg.before : []) {
    if (!Array.isArray(chain)) continue;
    const c = chain.slice(0, 2).map(normStation);
    c.forEach((s, i) => { if (s && s !== xfer && !before.has(s)) before.set(s, { hop: i + 1, approach: chain.slice(0, i + 1).reverse() }); });
  }

  const dirOk = (t) => {
    const terms = [normStation(t.statnTnm), normTerm(t.statnTnm)].filter(Boolean);
    if (!terms.length || terms.includes(xfer)) return false;          /* 환승역이 종착 — 더 가지 않는다 */
    if (loopDir) {
      if (terms.some((x) => offSet.has(x))) return false;             /* 순환선 지선 열차 */
      const u = String(t.updnLine == null ? "" : t.updnLine);
      if (u === "0" || u === "1") return u === loopDir;
    }
    return terms.some((x) => after.has(x) || dirSet.has(x));
  };

  const APPROACH_RANK = { "1": 0, "0": 1, "3": 2 };
  let best = null;
  for (const t of dedupByTrainNo(list)) {
    ...
    let rank, approach = [];
    if (at === xfer) rank = sttus === "1" ? 0 : sttus === "0" ? 1 : 2;   /* 환승역에 있음(진입 "0" 포함) */
    else if (before.has(at) && sttus in APPROACH_RANK) {                 /* 직전 역에서 다가오는 중 */
      const b = before.get(at);
      rank = 3 + (b.hop - 1) * 3 + APPROACH_RANK[sttus];
      approach = b.approach;
    } else continue;
    if (opts && typeof opts.skip === "function" && opts.skip(String(t.trainNo))) continue;   /* 운행 대기 열차 */
    if (!dirOk(t)) continue;                            /* 반대 방향·판단 불가 */
    if (!best || rank < best.rank) best = { t, rank, approach, sttus };
  }
  if (!best) return null;
  const t = best.t;
  return {
    ...
    approach: best.approach,                            /* 아직 환승역 전이면 그 역→환승역 경로(환승역 제외) */
    boardedAt: now,
...
/**
 * 시간 기준 진행(웹 tripTimeProgress 와 같은 생각) — 환승역에서 열차를 끝내 못 잡았을 때.
 * legs[base] 를 boardAt 에 탔다고 보고, 구간마다 min 을 역 수로 고르게 나눠 다음 역·남은 시간·도착 시각을 낸다.
 * 구간 사이(환승 ROUTE_TRANSFER_MIN) 동안은 그 환승역에서 '대기 중'으로 보인다.
 * @returns null 또는 { state, legIdx, onBaseLeg(아직 base 구간을 달리는 중), arrivalMs, done(도착 시각 지남) }
 */
function timeState(track, prev, base, boardAt, now = Date.now()) {
  const legs = Array.isArray(track.legs) ? track.legs : [];
  if (!legs[base]) return null;
  const lastLeg = legs.length - 1;
  const legMin = (i) => Math.max(0, Number(legs[i].min) || 0);
  const dur = (i) => legMin(i) + (i < lastLeg ? ROUTE_TRANSFER_MIN : 0);
  let total = 0;
  for (let i = base; i <= lastLeg; i++) total += dur(i);
  const arrivalMs = boardAt + total * 60000;
  const beforeBoard = now < boardAt;
  const elapsedMin = beforeBoard ? 0 : (now - boardAt) / 60000;

  let li = lastLeg, legStart = 0, acc = 0;
  for (let i = base; i <= lastLeg; i++) {
    if (elapsedMin < acc + dur(i) || i === lastLeg) { li = i; legStart = acc; break; }
    acc += dur(i);
  }
  const leg = legs[li];
  const segEl = elapsedMin - legStart;
  const sts = Array.isArray(leg.stations) && leg.stations.length ? leg.stations : [leg.to || ""];
  const n = sts.length;
  const hop = n > 1 ? legMin(li) / (n - 1) : 0;
  const inTransfer = !beforeBoard && li < lastLeg && segEl >= legMin(li);   /* 이 구간은 끝났고 다음 열차를 기다리는 중 */
  let idx = hop > 0 ? Math.floor(segEl / hop) + 1 : n - 1;
  idx = Math.max(0, Math.min(n - 1, beforeBoard ? 0 : idx));
  const remain = Math.max(0, Math.ceil((arrivalMs - now) / 60000 - 1e-9));
  const nl = inTransfer ? legs[li + 1] : null;
  const line = nl ? nl.line : leg.line;
  const state = Object.assign({}, prev || {}, {
    remainMin: remain,
    arriveAt: clockAfter(0, arrivalMs),
    line: line || (prev && prev.line) || "",
    colorHex: lineColor(line),
    nextStation: inTransfer ? (leg.to || sts[n - 1]) : beforeBoard ? sts[0] : (sts[idx] || leg.to || ""),
    legTo: nl ? (nl.to || "") : (leg.to || ""),
    isLast: nl ? li + 1 >= lastLeg : li >= lastLeg,
    done: false,
    waiting: beforeBoard || inTransfer,
    waitMin: beforeBoard ? Math.max(1, Math.ceil((boardAt - now) / 60000))
      : inTransfer ? Math.max(1, Math.ceil(legMin(li) + ROUTE_TRANSFER_MIN - segEl)) : 0,
    alight: !beforeBoard && !inTransfer && idx >= n - 1 && remain > 0,
    endEpoch: arrivalMs,
  });
  return { state, legIdx: li, onBaseLeg: li === base && !inTransfer, arrivalMs, done: now >= arrivalMs };
}
...
module.exports = {
  ...
  laterMinFrom, findTrain, applyTrack, waitingState, pickNextTrain, timeState, normTerm,
